...
class Debug_Bar_Queries extends Debug_Bar_Panel {
	...
	function render() {
		global $gpdb, $EZSQL_ERROR;

		$out = '';
		$total_time = 0;

		if ( ! empty( $gpdb->queries ) ) {
			$show_many = isset($_GET['debug_queries']);

			if ( $gpdb->num_queries > 500 && ! $show_many ) {
		...
		$heading = '';
		if ( $gpdb->num_queries ) {
			$heading .= '<h2><span>Total Queries:</span>' . number_format( $gpdb->num_queries ) . "</h2>\n";
		}
